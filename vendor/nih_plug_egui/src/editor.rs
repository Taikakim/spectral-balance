        ...
        let window = EguiWindow::open_parented(
            ...
            move |egui_ctx, queue, state| {
                ...
                if let Some(new_size) = egui_state.requested_size.swap(None) {
                    // IMPORTANT: store the new size into `egui_state.size` *before* calling
                    // `context.request_resize()`. The host will query the plugin's current
                    // size via the editor's `size()` method, which falls back to
                    // `egui_state.size` when `requested_size` is `None` (which it already is
                    // after the swap above). Without this, the host gets the *old* size and
                    // never actually resizes the parent window — shrinking leaves black
                    // borders on the right/bottom. See host side: clap ext_gui_get_size.
                    ...
                    if context.request_resize() {
                        ...
                        // Host refused the resize request — revert the stored size so it
                        // stays in sync with the actual window dimensions.
